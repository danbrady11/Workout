...
import { ALL_CALENDAR_TYPES, WORKOUT, ACTIVITY_TYPES } from '../data.js'
...
function SessionDetail({ sessionData }) {
    ...
    <div style={sd.wrap}>
      {WORKOUT.exercises.map(ex => {
        ...
        return (
          <div key={ex.id} style={sd.row}>
            ...
            <div style={sd.stats}>
              <span style={sd.badge}>{d.doneSets||0}×{d.reps||ex.repsDefault} reps</span>
      ...
      {sessionData._finisher?.completedRounds > 0 && (
        <div style={{ ...sd.row, borderBottom: 'none' }}>
          <div style={sd.name}>KB Finisher</div>
          <div style={sd.stats}>
            <span style={{ ...sd.badge, color: 'var(--legs)', borderColor: 'var(--legs)' }}>
              {sessionData._finisher.completedRounds}/3 rounds
...
export default function CalendarView({ calendarData, sessions, onCalendarChange }) {
  ...
  const { weeklyMins, monthlyMins } = useMemo(() => {
    ...
    Object.entries(calendarData).forEach(([key, entry]) => {
      const mins = entry.duration ? parseInt(entry.duration) : (entry.type === 'workout' ? LIFT_DEFAULT : 0)
  ...
  const selectedSession = selectedEntry?.type === 'workout' ? sessions?.[selectedDate] : null
  const isWorkoutType = editEntry?.type === 'workout'
    ...
    <div>
      ...
      <div style={styles.gridWrap}>
        ...
        <div style={styles.dayGrid}>
          {cells.map((day, i) => {
            ...
            const mins = entry?.duration ? parseInt(entry.duration) : (entry?.type==='workout' ? LIFT_DEFAULT : null)
            return (
      ...
      <div style={styles.recentSection}>
        ...
        {Object.entries(calendarData).sort(([a],[b])=>b.localeCompare(a)).slice(0,10).map(([key, entry]) => {
          ...
          const sess = entry.type==='workout' ? sessions?.[key] : null
          const mins = entry.duration ? parseInt(entry.duration) : (entry.type==='workout' ? LIFT_DEFAULT : null)
          return (
                ...
                {sess && (
                  <div style={styles.recentSets}>
                    {WORKOUT.exercises.map(ex => {
                      ...
                      return <span key={ex.id} style={styles.miniSet}>{ex.name.split(' ').pop()}: {d.doneSets}×{d.reps||ex.repsDefault} @ {d.weight||'?'}lb</span>
                    })}
                    {sess._finisher?.completedRounds > 0 && (
                      <span style={{ ...styles.miniSet, color:'var(--legs)' }}>KB: {sess._finisher.completedRounds}/3 rounds</span>
                    )}
                  </div>
                )}
              </div>
            ...
            <div style={styles.modalBody}>
              {selectedSession && (
                <div style={{ marginBottom:'1.25rem' }}>
                  ...
                  <div style={{ ...styles.sessionCard, borderColor:'var(--accent)' }}>
                    <SessionDetail sessionData={selectedSession} />
                  </div>
                </div>
              )}
